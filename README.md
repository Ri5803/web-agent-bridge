...
简单说，这个项目就是给主代理接上几个正在浏览器里工作的 GPT。
主代理可以随时创建新的网页 GPT 对话，把任务直接发过去，让它们分别做研究、
找反例、检查代码或整理资料。网页 GPT 做完以后，结果会自动通过 MCP
回到主代理，不需要你在多个聊天窗口之间来回复制粘贴，也不需要主代理一直轮询。

它不是把网页固定在某个标签页位置，而是可以按需创建、管理和关闭多个独立子代理。
每个子代理都有自己的对话上下文，还可以单独指定模型和思考强度。
对主代理来说，网页端 GPT 就像一个可以创建、发送任务、等待结果和继续对话的外部子代理。

Web Agent Bridge 通过本地 MCP 服务和浏览器扩展，把独立的 ChatGPT 网页对话
接入 Codex 工作流：自动创建新对话、并行分派任务、保留上下文续聊。
网页任务完成后，通过事件中转把原文结果回传给正在工作的主代理。
