...
很多人把 ChatGPT 网页当成一个只能手动点开的聊天窗口。
这个项目换了一个思路：让 Codex 可以随时召集几位正在浏览器里工作的网页 GPT，
把它们当成真正可以分工、协作、等待和回传结果的外部子代理。

## 这意味着什么？

你不必再自己开一排聊天窗口，也不必在多个对话之间复制粘贴。
你只需要告诉主代理：“让一个代理做研究，让另一个代理找反例，再让第三个代理检查代码。”
主代理就能按需创建新的网页 GPT 对话，把任务分别交出去；网页 GPT 完成后，
结果会通过 MCP 和事件中转自动回到主代理。

每个子代理都有独立的 ChatGPT 对话上下文，可以单独承担不同角色，
也可以在原对话里继续追问。它们不是固定绑在某个标签页上的脚本，
而是可以动态创建、并行工作、继续对话，最后按需关闭的代理。

换句话说：你原本只有一个需要亲自盯着的聊天窗口，
现在可以让主代理替你组织一支临时的网页 GPT 小队。

Web Agent Bridge 通过本地 MCP 服务和浏览器扩展，把独立的 ChatGPT 网页对话
接入 Codex 工作流。网页 GPT 仍然运行在你的浏览器和 ChatGPT 账号里，
但对主代理来说，它们已经变成了可以创建、发送任务、等待结果和继续对话的外部子代理。
